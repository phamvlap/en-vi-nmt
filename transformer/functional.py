...
def create_causal_mask(size: Tensor) -> Tensor:
    """
    Args
    ...
    """
    tril_mask = torch.tril(torch.ones(1, size, size), diagonal=0)
    return tril_mask
...
def create_encoder_mask(encoder_input: Tensor, pad_token_id: int) -> Tensor:
    """
    Args
        encoder_input: input tensor of encoder, shape `(seq_length, )`
        pad_token_id: token id of padding token
    Returns
        Tensor with shape `(1, 1, seq_length)`
    """
    encoder_mask = encoder_input != pad_token_id
    encoder_mask = encoder_mask.unsqueeze(0).unsqueeze(0).int().to(encoder_input.device)
    return encoder_mask
...
def create_decoder_mask(decoder_input: Tensor, pad_token_id: int) -> Tensor:
    """
    Args
        decoder_input: input tensor of decoder, shape `(seq_length, )`
        pad_token_id: token id of padding token
    Returns
        Tensor with shape `(1, seq_length, seq_length)`
    """
    ...
    decoder_mask = (decoder_mask.unsqueeze(0).unsqueeze(0).int() & causal_mask).to(
